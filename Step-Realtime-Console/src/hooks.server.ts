...
async function startWebSocketServer() {
  ...
  try {
    ...
    const wsPort = parseInt(env.WS_PORT || '8080');
    bunServer = Bun.serve({
      port: wsPort,
      fetch(req, server) {
        // 处理 WebSocket 升级请求
      ...
      websocket: {
        open(ws: any) {
          ...
          console.log(`Incoming connection URL: ${requestUrl}`);
          ...
          try {
            ...
            // 如果 URL 解析失败，尝试将其视为相对路径
            try {
               url = new URL(requestUrl, 'http://localhost');
               clientApiKey = url.searchParams.get('apiKey');
               clientModel = url.searchParams.get('model');
               console.log('Parsed URL as relative path');
            } catch (e2) {
               console.error('Error parsing URL as relative:', e2);
            }
          }

          if (clientApiKey) {
             console.log(`API Key received (length: ${clientApiKey.length}, starts with: ${clientApiKey.substring(0, 5)}...)`);
          } else {
             console.log('No API Key found in URL params');
          }
    ...
    console.log(`实时语音 WebSocket 中转服务器已启动，监听端口 ${wsPort}`);
  } catch (error) {
